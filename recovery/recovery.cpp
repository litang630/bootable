...
#define MIRA_BACKUP_OFFSET  // enable the MiraMEMS' backup (gsensor offset file) solution
#ifdef MIRA_BACKUP_OFFSET
static const char *BACKUP_FILE_PATH = "/data/misc/miraGSensorOffset.txt";
static const char *TEMP_BACKUP_PATH = "/tmp/miraGSensorOffset.txt";
#endif
...
#ifdef MIRA_BACKUP_OFFSET
static int mira_copy_file(const char *src,const char *dstFilePath)
{
    int BUFFER_SIZE = 512;
    ...
    if((read = fread(buf, 1, BUFFER_SIZE, fin)) > 0)
    {
    	if((buf[0]=='0')&&(buf[1]==' ')&&(buf[2]=='0')&&(buf[3]==' ')&&(buf[4]=='0'))
    	{
		 free(buf);
...
main(int argc, char **argv) {
  ...
  #ifdef MIRA_BACKUP_OFFSET
    mira_copy_file(BACKUP_FILE_PATH,TEMP_BACKUP_PATH);
    #endif
 ...
 #ifdef MIRA_BACKUP_OFFSET
    mira_copy_file(TEMP_BACKUP_PATH,BACKUP_FILE_PATH);
    chmod(BACKUP_FILE_PATH, 0666);
    chown(BACKUP_FILE_PATH, 1000, 1000); // system system
    #endif
//adupsfota start
#ifdef ADUPS_FOTA_SUPPORT
    if (perform_fota == 1) {
