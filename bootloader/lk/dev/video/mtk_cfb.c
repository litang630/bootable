...
#define SWAP32_16(x) ((((x) & 0x0000ffff) << 16) | (((x) & 0xffff0000) >>16)) //wisky-lxh@20150318
	...
	switch (data_fmt) 
	{
		...
		if(0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "180", 3))
		{
			while (count--){
				...
				{
						u8 bits = *pos--;
					((u32 *)tdest)[0] = SWAP32_16 ((lk_cfb_font_dtable16 [bits & 3] & cfb_eorx) ^ cfb_bgx);
					((u32 *)tdest)[1] = SWAP32_16 ((lk_cfb_font_dtable16 [bits >> 2 & 3] & cfb_eorx) ^ cfb_bgx);
					((u32 *)tdest)[2] = SWAP32_16 ((lk_cfb_font_dtable16 [bits >> 4 & 3] & cfb_eorx) ^ cfb_bgx);
					((u32 *)tdest)[3] = SWAP32_16 ((lk_cfb_font_dtable16 [bits >> 6] & cfb_eorx) ^ cfb_bgx);
				}
			pdest = pdest + MTK_VFW * PIXEL_SIZE;
			}
		}
		...
		if(0 == strncmp(MTK_LCM_PHYSICAL_ROTATION,"180",3))
  	{
	   while (count--) {
	    ...
	    {
	     u8 bits = *pos--;
	     ((u32 *) tdest)[7] = 0xff000000|SWAP32 ((lk_cfb_font_dtable32 [bits >> 4][0] & cfb_eorx) ^ cfb_bgx);
	     ((u32 *) tdest)[6] = 0xff000000|SWAP32 ((lk_cfb_font_dtable32 [bits >> 4][1] & cfb_eorx) ^ cfb_bgx);
	     ((u32 *) tdest)[5] = 0xff000000|SWAP32 ((lk_cfb_font_dtable32 [bits >> 4][2] & cfb_eorx) ^ cfb_bgx);
	     ((u32 *) tdest)[4] = 0xff000000|SWAP32 ((lk_cfb_font_dtable32 [bits >> 4][3] & cfb_eorx) ^ cfb_bgx);
	     ((u32 *) tdest)[3] = 0xff000000|SWAP32 ((lk_cfb_font_dtable32 [bits & 15][0] & cfb_eorx) ^ cfb_bgx);
	     ((u32 *) tdest)[2] = 0xff000000|SWAP32 ((lk_cfb_font_dtable32 [bits & 15][1] & cfb_eorx) ^ cfb_bgx);
	     ((u32 *) tdest)[1] = 0xff000000|SWAP32 ((lk_cfb_font_dtable32 [bits & 15][2] & cfb_eorx) ^ cfb_bgx);
	     ((u32 *) tdest)[0] = 0xff000000|SWAP32 ((lk_cfb_font_dtable32 [bits & 15][3] & cfb_eorx) ^ cfb_bgx);
	    }
	    pdest = pdest + MTK_VFW * PIXEL_SIZE;
	   }
 	 }
...
void video_putc_ext (const char c)
{
	...
	switch (c) 
	{
		...
		default:		
			// draw-the-char
			cfb_lk_putchar((CFB_COL - cfb_cols -1) * MTK_VFW,(CFB_ROW - cfb_rows) * MTK_VFH,c);
			cfb_cols++;
